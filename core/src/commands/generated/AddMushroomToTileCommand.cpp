...
void AddMushroomToTileCommand::execute() {
	...
	if (!internalMainUnit()) {
		throw std::runtime_error(
				"Transformation was not successfully executed: addMushroomToTile");
	}
	...
	if ((collectionhelpers::type_select < kara::Mushroom
			> (self->getTileAt(location)->getContents()).size() == 1)
			== false) {
		throw CommandConstraintException(
				"Violation of Postcondition: a mushroom has to be placed on the tile's location");
	}
...
bool AddMushroomToTileCommand::internalMainUnit() {

	// new instance: o2:Mushroom
	std::shared_ptr < kara::Mushroom > o2 = std::make_shared<kara::Mushroom>();
	...
	// create reference: CREATE o1:Tile -> contents -> o2:Mushroom
	executeAddReference(o1, mpw::Tile::cFeatureKeyContents, o2);

	// create reference: CREATE self:EditorWorld -> tileContents -> o2:Mushroom
...
std::shared_ptr<mpw::Tile> AddMushroomToTileCommand::execute_findO1(
